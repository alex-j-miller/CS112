...
public:
	...
	bool insertAfter(Item before, Item after);
	bool insertBefore(Item after, Item before);
...
template<class Item>
bool List<Item>::insertAfter(Item before, Item after) {

	int currIndex = 0;
	for (Node *ptr = myFirst; ptr; ptr = ptr->myNext) {
		if (ptr->myItem == before) {
			if(ptr->myNext == myLast) { // append
				Node *tmpPtr = new Node(after, nullptr);
				if (mySize == 0) {
					myFirst = tmpPtr;
				} else {
					myLast->myNext = tmpPtr;
				...
				Node * newNode = new Node(after, ptr->myNext);
				ptr->myNext = newNode;
			}
			mySize++;
			return true;
...
template<class Item>
bool List<Item>::insertBefore(Item after, Item before) {
	int currIndex = 0;
	for (Node *ptr = myFirst; ptr; ptr = ptr->myNext) {
		if(ptr->myNext->myItem == after) { //prepend
			if(currIndex == 0) {
				myFirst = new Node(before, myFirst);
				if (mySize == 0) {
					myLast = myFirst;
				}
			} else {
				Node * newNode = new Node(before, ptr->myNext);
				ptr->myNext = newNode;
			}
			mySize++;
			return true;
